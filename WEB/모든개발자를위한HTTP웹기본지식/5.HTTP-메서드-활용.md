...
- 파일 등록 /files/{filename} -> PUT
    - 이 경우에는 PUT 을 쓰는 게 알맞다.
    - 기존파일이 있으면 지우고, 없으면 새로생성!
...
- 스토어(Store)
    - 위에 클라이언트가 직접 리소스의 URI를 관리하는 것을 스토어라고 한다.
    - 클라이언트가 관리하는 리소스 저장소
...
> 회원 등록 폼과 회원 등록의 URI를 같게 사용하는 것을 권장
- 회원 조회 /members/{id} -> GET
- 회원 수정 폼 /members/{id}/edit -> GET
- 회원 수정 /members/{id}/edit, /members/{id} -> POST
> 회원 수정 폼과 회원 수정의 URI를 같게 사용하는 것을 권장
