...
const solution = (arr, total) => {
  ...
  for (let i = 0; i < result.length; i++) {
    let cnt = 1;
    let price = total - (result[i][0] / 2 + result[i][1]);
    let temp1 = [[result[i][0], result[i][1]]];

    for (let j = 0; j < result.length; j++) {
      if (i === j) continue;

      if (price >= result[j][0] + result[j][1]) {
        price = price - (result[j][0] + result[j][1]);
        temp1.push([result[j][0], result[j][1]]);
        cnt++;
      } else {
        break;
      }
