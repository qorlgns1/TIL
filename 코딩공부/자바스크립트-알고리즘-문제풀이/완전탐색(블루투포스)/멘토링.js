...
// const solution = (학생수, 테스트결과, arr) => {
//   let answer = new Set();
//   let answer2 = new Set();

//   for (let i = 0; i < 테스트결과; i++) {
//     let result = arr[i];
//     for (let j = 0; j < 학생수; j++) {
//       for (let k = j + 1; k < 학생수; k++) {
//         answer.add(`${result[j]},${result[k]}`);
//         answer2.add(`${result[학생수 - j - 1]},${result[학생수 - k - 1]}`);
//       }
//     }
//   }

//   for (const x of answer2) {
//     answer.delete(x);
//   }
...
//   [3, 1, 4, 2],
// ];
// console.log(solution(arr[0].length, arr.length, arr));
...
for (let i = 0; i < test.length; i++) {
  ...
  for (let j = 0; j < arr.length; j++) {
    for (let k = j + 1; k < arr.length; k++) {
      set1.add(JSON.stringify([arr[j], arr[k]]));
      set2.add(
        JSON.stringify([arr[arr.length - 1 - j], arr[arr.length - 1 - k]])
      );
    }
  }
}

for (let item of set2) set1.delete(item);
